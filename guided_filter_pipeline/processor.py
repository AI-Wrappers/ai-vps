...
class MaskPairWorkloadProcessor(BaseWorkloadProcessor):
    ...
    def process(self, raw_workload: Any) -> Iterable[BatchTask]:
        if not isinstance(raw_workload, dict):
            raise ValueError("Workload must be a dictionary containing 'src_masks', 'src_images', and 'dst'.")
        ...
        if not src_masks or not src_images or not dst:
            raise ValueError("Workload must specify 'src_masks', 'src_images' and 'dst' paths.")
        ...
        for mask_path in sorted(masks_root.glob("**/*")):
            if mask_path.is_file() and mask_path.suffix.lower() in supported_extensions:
                if "_mask_1024" in mask_path.name:
                    base_name = mask_path.name.replace("_mask_1024" + mask_path.suffix, "")
                    rel_dir = mask_path.relative_to(masks_root).parent
                    
                    expected_img_name = f"{base_name}_upscale_4k.png"
                    img_path = images_root / rel_dir / expected_img_name
                    
                    if img_path.exists():
                        all_items.append(MaskPairItem(
                            mask_1k_path=str(mask_path),
                            upscale_4k_path=str(img_path),
                            relative_path=str(rel_dir / base_name)
                        ))
                    else:
                        logger.warning(f"Found mask {mask_path} but missing upscaled image {img_path}")
        ...
        logger.info(f"Scanned and created {len(batches)} batches (batch size: {batch_size}).")
        return batches
